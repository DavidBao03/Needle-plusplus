...
def test_power_scalar_forward():
    np.testing.assert_allclose(
        ndl.power_scalar(ndl.Tensor([[0.5, 2.0, 3.0]], device=nd.cpu()), scalar=2).numpy(),
...
def test_divide_forward():
    np.testing.assert_allclose(
        ndl.divide(
            ndl.Tensor([[3.3, 4.35, 1.2], [2.45, 0.95, 2.55]], device=nd.cpu()),
            ndl.Tensor([[4.6, 4.35, 4.8], [0.65, 0.7, 4.4]], device=nd.cpu()),
        ).numpy(),
        np.array(
            [
...
def test_divide_scalar_forward():
    np.testing.assert_allclose(
        ndl.divide_scalar(ndl.Tensor([[1.7, 1.45]], device=nd.cpu()), scalar=12).numpy(),
...
def test_matmul_forward():
    np.testing.assert_allclose(
        ndl.matmul(
            ndl.Tensor([[4.95, 1.75, 0.25], [4.15, 4.25, 0.3], [0.3, 0.4, 2.1]], device=nd.cpu()),
            ndl.Tensor([[1.35, 2.2, 1.55], [3.85, 4.8, 2.6], [1.15, 0.85, 4.15]], device=nd.cpu()),
        ).numpy(),
        np.array(
    ...
    np.testing.assert_allclose(
        ndl.matmul(
            ndl.Tensor([[3.8, 0.05], [2.3, 3.35], [1.6, 2.6]], device=nd.cpu()),
            ndl.Tensor([[1.1, 3.5, 3.7], [0.05, 1.25, 1.0]], device=nd.cpu()),
        ).numpy(),
        np.array(
    ...
    np.testing.assert_allclose(
        ndl.matmul(
            ndl.Tensor([[1.9, 1.9], [4.8, 4.9], [3.25, 3.75]], device=nd.cpu()),
            ndl.Tensor(
                [
    ...
    np.testing.assert_allclose(
        ndl.matmul(
            ...
            ndl.Tensor([[2.2, 0.65, 2.5], [2.5, 1.3, 0.15]], device=nd.cpu()),
        ).numpy(),
        np.array(
            [
                [
...
def test_summation_forward():
    ...
    np.testing.assert_allclose(
        ndl.summation(
            ndl.Tensor([[1.5, 3.85, 3.45], [1.35, 1.3, 0.65], [2.6, 4.55, 0.25]], device=nd.cpu()),
            axes=0,
        ).numpy(),
...
def test_broadcast_to_forward():
    np.testing.assert_allclose(
        ndl.broadcast_to(ndl.Tensor([[1.85, 0.85, 0.6]], device=nd.cpu()), shape=(2, 3)).numpy(),
        np.array(
...
def test_transpose_forward():
    np.testing.assert_allclose(
        ndl.transpose(ndl.Tensor([[[1.95]], [[2.7]], [[3.75]]], device=nd.cpu()), axes=(1, 2)).numpy(),
        np.array([[[1.95]], [[2.7]], [[3.75]]]),
    )
    np.testing.assert_allclose(
        ndl.transpose(
            ndl.Tensor([[[[0.95]]], [[[2.55]]], [[[0.45]]]], device=nd.cpu()), axes=(2, 3)
        ).numpy(),
    ...
    np.testing.assert_allclose(
        ndl.transpose(ndl.Tensor([[[2.45]], [[3.5]], [[0.9]]], device=nd.cpu()), axes=(0, 1)).numpy(),
        np.array([[[2.45], [3.5], [0.9]]]),
    )
    np.testing.assert_allclose(
        ndl.transpose(ndl.Tensor([[4.4, 2.05], [1.85, 2.25], [0.15, 1.4]], device=nd.cpu())).numpy(),
        np.array([[4.4, 1.85, 0.15], [2.05, 2.25, 1.4]]),
    )
    np.testing.assert_allclose(
        ndl.transpose(
            ndl.Tensor([[0.05, 3.7, 1.35], [4.45, 3.25, 1.95], [2.45, 4.4, 4.5]], device=nd.cpu())
        ).numpy(),
...
def test_log_forward():
    np.testing.assert_allclose(
        ndl.log(ndl.Tensor([[4.0], [4.55]], device=nd.cpu())).numpy(),
...
def test_exp_forward():
    np.testing.assert_allclose(
        ndl.exp(ndl.Tensor([[4.0], [4.55]], device=nd.cpu())).numpy(),
        np.array([[54.59815003],[94.63240831]]), rtol=1e-05, atol=1e-5
    )
...
def test_ewisepow_forward():
    np.testing.assert_allclose(
        ndl.power(
            ndl.Tensor([[1.0, 2.0, 3.0]], device=nd.cpu()),
            ndl.Tensor([[0, 0, 2]], device=nd.cpu()),
        ).numpy(),
...
def gradient_check(f, *args, tol=5e-4, backward=False, **kwargs):
    eps = 1e-2
    numerical_grads = [np.zeros(a.shape).astype(np.float32) for a in args]
    ...
    if not backward:
        ...
        computed_grads = [
            x.numpy()
            for x in out.op.gradient_as_tuple(ndl.Tensor(np.ones(out.shape), device=nd.cpu(), dtype="float32"), out)
        ]
    else:
...
def test_divide_backward():
    gradient_check(
        ndl.divide,
        ndl.Tensor(np.random.randn(5, 4), device=nd.cpu(), dtype="float32"),
        ndl.Tensor(5 + np.random.randn(5, 4), device=nd.cpu(), dtype="float32"),
    )


def test_divide_scalar_backward():
    gradient_check(
        ndl.divide_scalar, ndl.Tensor(np.random.randn(5, 4), device=nd.cpu(), dtype="float32"), scalar=np.random.randn(1)
    )


def test_matmul_simple_backward():
    gradient_check(
        ndl.matmul, ndl.Tensor(np.random.randn(5, 4), device=nd.cpu(), dtype="float32"), ndl.Tensor(np.random.randn(4, 5), device=nd.cpu(), dtype="float32")
    )
...
def test_matmul_batched_backward():
    gradient_check(
        ndl.matmul,
        ndl.Tensor(np.random.randn(6, 6, 5, 4), device=nd.cpu(), dtype="float32"),
        ndl.Tensor(np.random.randn(6, 6, 4, 3), device=nd.cpu(), dtype="float32"),
    )
    gradient_check(
        ndl.matmul,
        ndl.Tensor(np.random.randn(6, 6, 5, 4), device=nd.cpu(), dtype="float32"),
        ndl.Tensor(np.random.randn(4, 3), device=nd.cpu(), dtype="float32"),
    )
    gradient_check(
        ndl.matmul,
        ndl.Tensor(np.random.randn(5, 4), device=nd.cpu(), dtype="float32"),
...
def test_reshape_backward():
    gradient_check(ndl.reshape, ndl.Tensor(np.random.randn(5, 4), device=nd.cpu(), dtype="float32"), shape=(4, 5))


def test_negate_backward():
    gradient_check(ndl.negate, ndl.Tensor(np.random.randn(5, 4), device=nd.cpu(), dtype="float32"))


def test_transpose_backward():
    gradient_check(ndl.transpose, ndl.Tensor(np.random.randn(3, 5, 4), device=nd.cpu(), dtype="float32"), axes=(1, 2))
    gradient_check(ndl.transpose, ndl.Tensor(np.random.randn(3, 5, 4), device=nd.cpu(), dtype="float32"), axes=(0, 1))


def test_broadcast_to_backward():
    gradient_check(ndl.broadcast_to, ndl.Tensor(np.random.randn(3, 1), device=nd.cpu(), dtype="float32"), shape=(3, 3))
    gradient_check(ndl.broadcast_to, ndl.Tensor(np.random.randn(1, 3), device=nd.cpu(), dtype="float32"), shape=(3, 3))
    gradient_check(
        ndl.broadcast_to,
        ndl.Tensor(
            np.random.randn(
                3,
            ), device=nd.cpu(), dtype="float32"
        ),
        shape=(3, 3),
    )
    gradient_check(ndl.broadcast_to, ndl.Tensor(np.random.randn(), device=nd.cpu(), dtype="float32"), shape=(3, 3, 3))
    gradient_check(
        ndl.broadcast_to, ndl.Tensor(np.random.randn(5, 4, 1), device=nd.cpu(), dtype="float32"), shape=(5, 4, 3)
    )


def test_summation_backward():
    gradient_check(ndl.summation, ndl.Tensor(np.random.randn(5, 4), device=nd.cpu(), dtype="float32"), axes=(1,))
    gradient_check(ndl.summation, ndl.Tensor(np.random.randn(5, 4), device=nd.cpu(), dtype="float32"), axes=(0,))


def test_log_backward():
    gradient_check(ndl.log, ndl.Tensor(1 + np.random.rand(5, 4), device=nd.cpu(), dtype="float32"))


def test_exp_backward():
    gradient_check(ndl.exp, ndl.Tensor(1 + np.random.rand(5, 4), device=nd.cpu(), dtype="float32")) 


def test_ewisepow_backward():
    gradient_check(
        ndl.power, ndl.Tensor([[1.0, 2.0, 3.0]], device=nd.cpu(), dtype="float32"), ndl.Tensor(np.ones((3,))*2, device=nd.cpu(), dtype="float32")
    )
...
def test_topo_sort():
    # Test case 1
    a1, b1 = ndl.Tensor(np.asarray([[0.88282157]]), device=nd.cpu(), dtype="float32"), ndl.Tensor(
        np.asarray([[0.90170084]]), device=nd.cpu(), dtype="float32"
    )
    ...
    # Test case 3
    a = ndl.Tensor(np.asarray([[1.4335016, 0.30559972], [0.08130171, -1.15072371]]), device=nd.cpu(), dtype="float32")
    b = ndl.Tensor(np.asarray([[1.34571691, -0.95584433], [-0.99428573, -0.04017499]]), device=nd.cpu(), dtype="float32")
    e = (a @ b + b - a) @ a
...
def test_compute_gradient():
    gradient_check(
        ...
        ndl.Tensor(np.random.randn(10, 9), device=nd.cpu(), dtype="float32"),
        ndl.Tensor(np.random.randn(9, 8), device=nd.cpu(), dtype="float32"),
        ndl.Tensor(np.random.randn(10, 8), device=nd.cpu(), dtype="float32"),
        backward=True,
    )
    gradient_check(
        lambda A, B: ndl.summation(ndl.broadcast_to(A, shape=(10, 9)) * B, axes=None),
        ndl.Tensor(np.random.randn(10, 1), device=nd.cpu(), dtype="float32"),
        ndl.Tensor(np.random.randn(10, 9), device=nd.cpu(), dtype="float32"),
        backward=True,
    )
    gradient_check(
        ...
        ndl.Tensor(np.random.randn(100), device=nd.cpu(), dtype="float32"),
        ndl.Tensor(np.random.randn(10, 5), device=nd.cpu(), dtype="float32"),
        ndl.Tensor(np.random.randn(10, 5), device=nd.cpu(), dtype="float32"),
        backward=True,
    )

    # check gradient of gradient
    x2 = ndl.Tensor([6], device=nd.cpu(), dtype="float32")
    x3 = ndl.Tensor([0], device=nd.cpu(), dtype="float32")
    y = x2 * x2 + x2 * x3
    y.backward()
    grad_x2 = x2.grad
    grad_x3 = x3.grad
...
def test_softmax_loss_ndl():
    ...
    np.testing.assert_allclose(
        ndl.log(ndl.Tensor([[4.0], [4.55]], device=nd.cpu(), dtype="float32")).numpy(),
        np.array([[1.38629436112], [1.515127232963]]),
    )
    ...
    np.random.seed(0)
    Z = ndl.Tensor(np.zeros((y.shape[0], 10)), device=nd.cpu(), dtype="float32")
    y_one_hot = np.zeros((y.shape[0], 10))
    y_one_hot[np.arange(y.size), y] = 1
    y = ndl.Tensor(y_one_hot, device=nd.cpu(), dtype="float32")
    np.testing.assert_allclose(
        softmax_loss(Z, y).numpy(), 2.3025850, rtol=1e-5, atol=1e-5
    )
    Z = ndl.Tensor(np.random.randn(y.shape[0], 10), device=nd.cpu(), dtype="float32")
    np.testing.assert_allclose(
        softmax_loss(Z, y).numpy(), 2.7291998, rtol=1e-5, atol=1e-5
    )

    # test softmax loss backward
    Zsmall = ndl.Tensor(np.random.randn(16, 10).astype(np.float32), device=nd.cpu(), dtype="float32")
    ysmall = ndl.Tensor(y_one_hot[:16], device=nd.cpu(), dtype="float32")
...
def test_nn_epoch_ndl():
    ...
    gradient_check(ndl.relu, ndl.Tensor(np.random.randn(5, 4), device=nd.cpu(), dtype="float32"))
